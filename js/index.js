...
$(document).ready(function () {
  ...
  function dialogHandler() {
    ...
    } else $('.dialog').text('Player ' + whoMovesNow + ' moves');
  }
  ...
  function play() {
    ...
    if (gameOver) resetGame();else if (JSboard[cell] === '-') {
      // Here we move
  ...
  function isGameOver() {
    return ['X', 'Y'].find(function (player) {
      return winPermutations.some(function (perm) {
        return perm.every(function (cell) {
          return playerToArr(player).includes(cell);
        });
      });
    }) || isBoardFull();
  }
  ...
  function isBoardFull() {
    if (JSboard.filter(function (cell) {
      return cell !== '-';
    }).length === 9) return 'Nobody';
  }
